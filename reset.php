...
if(isset($_POST['submit'])){
	...
	if(!isset($error)){
		...
		try {
			...
			$stmt = $pdo->prepare("UPDATE t_user SET resetToken = :token, resetComplete='No' WHERE email = :email");
			$stmt->execute(array(
				':email' => $row['email'],
				':token' => $token
			));
			...
			$body = 'Someone requested that the password be reset. \n\nIf this was a mistake, just ignore this email and nothing will happen.\n\n To reset your password, visit the following address: http://'.DIR.'resetPassword.php?key='.$token.'';
			$additionalheaders = "From: <".SITEEMAIL.">\r\n";
			$additionalheaders .= "Reply-To: $".SITEEMAIL."";
					
			$retval = mail ($to,$subject,$body,$additionalheaders);
